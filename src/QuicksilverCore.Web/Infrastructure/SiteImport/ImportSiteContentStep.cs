...
using QuicksilverCore.Web.Features.Payment.Services;
using ILogger = EPiServer.Logging.ILogger;
using SearchOptions = Mediachase.Search.SearchOptions;

namespace QuicksilverCore.Web.Infrastructure.SiteImport;
...
public class ImportSiteContentStep : IMigrationStep
{
    ...
    private readonly ILogger _logger = LogManager.GetLogger(typeof(ImportSiteContentStep));
    ...
    public bool Execute(IProgressMessenger progressMessenger)
    {
        ...
        try
        {
            ...
            ImportAssets(Path.Combine(ServiceLocator.Current.GetInstance<IWebHostEnvironment>().ContentRootPath, @"ImportData\ProductAssets.episerverdata"));
            ...
            ImportCatalog(Path.Combine(ServiceLocator.Current.GetInstance<IWebHostEnvironment>().ContentRootPath, @"ImportData\Catalog_Fashion.zip"));
            ...
            BuildIndex(Mediachase.Commerce.Core.AppContext.Current.ApplicationName, true);
    ...
    {
        _taxImportExport.Service.Import(Path.Combine(ServiceLocator.Current.GetInstance<IWebHostEnvironment>().ContentRootPath, @"ImportData\Taxes.csv"), ',');
    }
    ...
    private void ImportAssets(string path)
    {
        ...
        var cache = ServiceLocator.Current.GetInstance<IMemoryCache>();
        ...
        foreach (var key in cache.GetKeys())
        {
            ...
            catch (Exception ex)
            {
                ...
                _logger.Error("Failed to clear cache key", ex);
            }
    ...
    {
        var searchManager = new SearchManager(applicationName, ServiceLocator.Current.GetInstance<IOptions<SearchOptions>>(), ServiceLocator.Current.GetInstance<IServiceProvider>(), ServiceLocator.Current.GetInstance<IndexBuilder>());
    ...
    private static void SyncMetaClassesToContentTypeModels()
    {
        ...
        tasks.AddRange(contentScanner.Sync(true));
